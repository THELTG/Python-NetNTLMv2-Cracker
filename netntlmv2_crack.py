...
python netntlmv2_crack.py /usr/share/wordlists/rockyou.txt  'Vulnerable User::VulnerableFYP:aaaaaaaaaaaaaaaa:4f0266fa915f2a97779c3a5618eeec10:'
''')
    exit(1)
...
def brutecrack_netntlmv2(final:dict) -> bool | str:
    ...
    for password in passwordlist:
        ...
        if final['proof'] == HMAC.new(k2,final['server_challenge'] + final['blob']).digest(): # check against the proof bytes
            print(f'[+] Cracked the hash: {password.decode("utf-16-le")}')
            break
        ctr += 1
        if ctr % approx == 0:
